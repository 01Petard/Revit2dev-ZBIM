    ...
    public class H00047 : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            */
            ...
            foreach (string buildFlag in buildNumList)
            {
                ...
                if (build_height > 54)
                {
                    foreach (Level litem in build_upfloorsNumList) //从这里开始遍历建筑的楼层
                    {
                        ...
                        {
                            string area_level = SubBetweenString(area.Name, "-", "F"); //得到该防火分区所在的层数
                            string area_firearea = area.LookupParameter("面积").AsValueString(); //得到Area指向的防火分区的面积，类似这种字段：“114514”
                            if (area_level == level_name_num.ToString()) //当防火分区的层数==当前层数时，遍历该楼所有高度符合的喷淋管的四维坐标是否在防火分区四维坐标范围内 
                            {
                                ...
                                foreach (Element FirePipe in FirePipe_buildNum[buildFlag])
                                {
                                    ...
                                    {
                                        exist_FirePipe = true;
                                        //判断该层是否有喷淋管，然后将该喷淋管的四维坐标与防火分区四维坐标作比较，看是否有管子在该防火分区四维坐标范围内
                                    }
                                    if (exist_FirePipe == true)//如果该楼层有喷淋管，则进行以下判断
                                    {
                                        //判断喷淋管是否在返货分区内
                                        if (FireAreaMinX < FirePipeMinX && FireAreaMinY < FirePipeMinY && FireAreaMaxX > FirePipeMinX && FireAreaMaxY < FirePipeMaxY)  // 如果喷淋管在防火分区范围内，大于3000才不符合
                                        {
                                            if (Convert.ToDouble(area_firearea) > 3000)
                                            {
                                                stringBuilder_notpass.AppendLine("Id：" + area.Id + "，名字：" + area.Name + "，面积：" + area_firearea + "㎡");
                                            }
                                            else
                                            {
                                                stringBuilder_pass.AppendLine("Id：" + area.Id + "，名字：" + area.Name + "，面积：" + area_firearea + "㎡");
                                            }
                                        }
                                        else // 如果喷淋管不在防火分区范围内，大于1500就不符合
                                        {
                                            if (Convert.ToDouble(area_firearea) > 1500)
                                            {
                                                stringBuilder_notpass.AppendLine("Id：" + area.Id + "，名字：" + area.Name + "，面积：" + area_firearea + "㎡");
                                            }
                                            else
                                    ...
                                    else// 如果没喷淋管，则进行以下判断，大于1500就不符合
                                    {
                                        if (Convert.ToDouble(area_firearea) > 1500)
                                        {
                                            stringBuilder_notpass.AppendLine("Id：" + area.Id + "，名字：" + area.Name + "，面积：" + area_firearea + "㎡");
                        ...
                        //判断该层是否有喷淋系统，同时得到喷淋管的MinX、MinY、MaxX、MaxY
                        foreach (Element FirePipe in FirePipe_buildNum[buildFlag])
                        {
                            FirePipeMinX = FirePipe.get_BoundingBox(document.ActiveView).Min.X;
                            FirePipeMaxX = FirePipe.get_BoundingBox(document.ActiveView).Max.X;
                        ...
                        //判断该楼的该层是否有防火分区，buildFlag代表楼号，level_name_num代表层号
                        foreach (Area area in FireArea_buildNum[buildFlag])
                        {
                            string area_level = SubBetweenString(area.Name, "-", "F"); //得到该防火分区所在的层数
                            string area_firearea = area.LookupParameter("面积").AsValueString(); //得到Area指向的防火分区的面积，类似这种字段：“114514”
                            if (area_level == level_name_num.ToString()) //当防火分区的层数==当前层数时，判断防火分区是否符合要求
                            {
                                if (exist_FirePipe == true) // 大于54m，若有喷淋则大于3000㎡归为不符
                                {
                                    if (Convert.ToDouble(area_firearea) > 3000)
                                    {
                                        stringBuilder_notpass.AppendLine("Id：" + area.Id + "，名字：" + area.Name + "，面积：" + area_firearea + "㎡");
                ...
                else if (build_height > 27 && build_height <= 54)
                {
                    foreach (Level litem in build_upfloorsNumList) //从这里开始遍历建筑的楼层
                    {
                        ...
                        foreach (Area area in FireArea_buildNum[buildFlag])
                        {
                            ...
                            if (area_level == level_name_num.ToString()) //当防火分区的层数==当前层数时，判断防火分区是否符合要求
                            {
                                if (exist_FirePipe == true) // 大于27m小于54m，若有喷淋则大于3000㎡归为不符
                                {
                                    if (Convert.ToDouble(area_firearea) > 3000)
                                    {
                                        stringBuilder_notpass.AppendLine("Id：" + area.Id + "，名字：" + area.Name + "，面积：" + area_firearea + "㎡");
                                ...
                                else // 大于27m小于54m，若没有喷淋则大于1500㎡归为不符
                                {
                                    if (Convert.ToDouble(area_firearea) > 1500)
                                    {
                                        stringBuilder_notpass.AppendLine("Id：" + area.Id + "，名字：" + area.Name + "，面积：" + area_firearea + "㎡");
                ...
                else
                {
                    foreach (Level litem in build_upfloorsNumList) //从这里开始遍历建筑的楼层
                    {
                        ...
                        foreach (Area area in FireArea_buildNum[buildFlag])
                        {
                            ...
                            if (area_level == level_name_num.ToString()) //当防火分区的层数==当前层数时，判断防火分区是否符合要求
                            {
                                if (exist_FirePipe == true) // 小于27m，若有喷淋则大于5000㎡归为不符
                                {
                                    if (Convert.ToDouble(area_firearea) > 5000)
                                    {
                                        stringBuilder_notpass.AppendLine("Id：" + area.Id + "，名字：" + area.Name + "，面积：" + area_firearea + "㎡");
                                        H00047_pass = false;
                                    }
                                ...
                                else // 小于27m，若没有喷淋则大于2500㎡归为不符
                                {
                                    if (Convert.ToDouble(area_firearea) > 2500)
                                    {
                                        stringBuilder_notpass.AppendLine("Id：" + area.Id + "，名字：" + area.Name + "，面积：" + area_firearea + "㎡");
                                        H00047_pass = false;
                                    }
            ...
            //打印输出
            //PrintLog(stringBuilder.ToString(), "H00047", document);
