...
namespace ZBIM
{
    ...
    public class H00099_boundingXYZ : IExternalCommand
    {
        //长度转换，“米”转“英尺”
        private static double metre_to_inch(double metre)
        {
            double inch = 0;
        ...
        private static string LastSubEndString(string str, string endStr, bool isContains = false, bool isIgnoreCase = true)
        {
        ...
        //打印日志到桌面
        public static void PrintLog(string info, string itemNum, Autodesk.Revit.DB.Document doc, string path = null)
        {
            // 获取模型编号M0000x
        ...
        public static Dictionary<string, List<Area>> GetBuildArea(Autodesk.Revit.DB.Document document, string AreaTypeName)
        {
            ...
            foreach (Area area in AreaCollector)
            {
                if (area.AreaScheme.Name.Contains(AreaTypeName))
                {
                    ...
                    {
                        area_buildnum = LastSubEndString(area.Name, AreaTypeName);//先分离出防火分区后面的字段，例如“B1-1 1”
                        area_buildnum = StartSubString(area_buildnum, 0, "-", false, true);//然后获得楼号，例如“B1”
                    ...
                    {
                        area_buildnum = LastSubEndString(area.Name, AreaTypeName);//先分离出防火分区后面的字段，例如“1#-2F-1 10”
                        area_buildnum = StartSubString(area_buildnum, 0, "#-");//然后获得楼号，例如“1”
                        FireArea_buildNum[area_buildnum].Add(area);
                        //stringBuilder.AppendLine("防火分区Id：" + area.Id + "，名称：" + area.Name + "，楼号：" + area_buildnum + "#");
                        area_num++;
                    }
        ...
        public static Dictionary<string, List<Pipe>> GetBuildPipe(Autodesk.Revit.DB.Document document, List<Pipe> PipeList, List<Element> elementsGridsSort)
        {
                    ...
                    !item.Name.Contains("测"))
                {
                    ...
                    else //如果前后两个轴网发生变化，说明已经切换到下一栋楼了，打印输出一下这栋楼的占地方位
                    {
                        ...
                        foreach (var Pipe in PipeList)
                        {
                            ...
                            PipeXLength = Math.Round(inch_to_metre(Math.Abs(PipeMaxX - PipeMinX)), 2);
                            PipeYLength = Math.Round(inch_to_metre(Math.Abs(PipeMaxY - PipeMinY)), 2);
                            PipeZLength = Math.Round(inch_to_metre(Math.Abs(PipeMaxZ - PipeMinZ)), 2);
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            //==========防火分区FireArea==========
            Dictionary<string, List<Area>> FireArea_buildNum = GetBuildArea(document, "防火分区");
            //查看每栋楼防火分区
            ...
            //==========找到每个轴网范围内的喷淋管道==========
            Dictionary<string, List<Pipe>> FirePipe_buildNum = GetBuildPipe(document, FirePipeList, elementsGridsSort);
            //查看每栋楼的喷淋管
