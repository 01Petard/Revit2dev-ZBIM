...
namespace ZBIM
{
    ...
    public class H00018_v1 : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            */
            ...
            foreach (String buildFlag in buildNumList)//从这里开始遍历楼栋
            {
                ...
                foreach (var litem in build_upfloorsNumList)
                {
                    stringBuilder.AppendLine("litem.Name：" + litem.Name + "litem.Name：" + litem.Name);
                }
                */
                //int levelNum = 0;

                StringBuilder stringBuilder_pass_FireGate = new StringBuilder();      //用于输出
                StringBuilder stringBuilder_not_pass_FireGate = new StringBuilder();  //用于输出
                StringBuilder stringBuilder_pass_FireAlarm = new StringBuilder();      //用于输出
                StringBuilder stringBuilder_not_pass_FireAlarm = new StringBuilder();  //用于输出
                StringBuilder stringBuilder_pass_FirePipe = new StringBuilder();       //用于输出
                StringBuilder stringBuilder_not_pass_FirePipe = new StringBuilder();   //用于输出
                ...
                String temp_level_name = "";//仅用于记录楼层的名字，1#、2#、3#
                ...
                else if (build_height > 21 && build_height <= 27) //21-27m的建筑，每层判断有无消防立管、消火栓
                {
                    foreach (Level litem in build_upfloorsNumList) //从这里开始遍历建筑的楼层
                    {
                        ...
                        pass_FirePipe = "";
                        //levelNum++;
                        temp_level_name = litem.Name;
                        while (temp_level_name.Contains("#"))
                        {
                            temp_level_name = SubBetweenString(temp_level_name, "#", "F", false, true, true);
                        }
                        //判断消火栓
                        ...
                        if (exist_FireGate == true)
                        {
                            stringBuilder_pass_FireGate.AppendLine(temp_level_name + "，有消火栓：" + pass_FireGate);
                        }
                        else
                        {
                            //stringBuilder_not_pass_FireGate.AppendLine(temp_level_name + "，无消火栓");
                            stringBuilder_not_pass_FireGate.Append(temp_level_name + "  ");
                            H00018_pass = false;
                        }
                        ...
                        {
                            stringBuilder_pass_FirePipe.AppendLine(temp_level_name + "，有消防立管：" + pass_FirePipe);
                        }
                        else
                        {
                            //stringBuilder_not_pass_FirePipe.AppendLine(temp_level_name + "，无消防立管");
                            stringBuilder_not_pass_FirePipe.Append(temp_level_name + "  ");
                            H00018_pass = false;
                        }
                    }

                    //输出消火栓的stringbuilder
                    if (!(stringBuilder_pass_FireGate.Length > 0 && !"null".Equals(stringBuilder_pass_FireGate.ToString()) && !"".Equals(stringBuilder_pass_FireGate.ToString())))
                    {
                        stringBuilder_pass_FireGate.AppendLine("无");
                    }
                    if (!(stringBuilder_not_pass_FireGate.Length > 0 && !"null".Equals(stringBuilder_not_pass_FireGate.ToString()) && !"".Equals(stringBuilder_not_pass_FireGate.ToString())))
                    {
                        stringBuilder_not_pass_FireGate.AppendLine("无");
                    }

                    if (exist_FireGate == false)
                    {
                        stringBuilder.Append("已设置“消火栓”的楼层：" + "\n" + stringBuilder_pass_FireGate);
                        stringBuilder.Append("未设置“消火栓”的楼层：" + "\n" + stringBuilder_not_pass_FireGate + "\n");
                    }
                    else
                    {
                        stringBuilder.Append("已设置“消防立管”的楼层：" + "\n" + stringBuilder_pass_FirePipe);
                    }


                    //输出消防立管的stringbuilder
                    if (!(stringBuilder_pass_FirePipe.Length > 0 && !"null".Equals(stringBuilder_pass_FirePipe.ToString()) && !"".Equals(stringBuilder_pass_FirePipe.ToString())))
                    {
                        stringBuilder_pass_FirePipe.AppendLine("无");
                    }
                    if (!(stringBuilder_not_pass_FirePipe.Length > 0 && !"null".Equals(stringBuilder_not_pass_FirePipe.ToString()) && !"".Equals(stringBuilder_not_pass_FirePipe.ToString())))
                    {
                        stringBuilder_not_pass_FirePipe.AppendLine("无");
                    }
                    if (exist_FirePipe == false)
                    {
                        stringBuilder.Append("已设置“消防立管”的楼层：" + "\n" + stringBuilder_pass_FirePipe);
                        stringBuilder.Append("未设置“消防立管”的楼层：" + "\n" + stringBuilder_not_pass_FirePipe + "\n");
                    }
                    else
                    {
                        stringBuilder.Append("已设置“消防立管”的楼层：" + "\n" + stringBuilder_pass_FirePipe);
                ...
                else if (build_height > 27) //大于27m的建筑，每层判断有无消火栓箱、消防立管、消火栓
                {
                    foreach (Level litem in build_upfloorsNumList)//从这里开始遍历建筑的楼层
                    {
                        //stringBuilder.AppendLine("litem.Name:" + litem.Name + "，litem.Id:" + litem.Id);
                        ...
                        {
                            stringBuilder_pass_FireAlarm.AppendLine(temp_level_name + "，有消火栓箱：" + pass_FireAlarm);
                        }
                        else
                        {
                            //stringBuilder_not_pass_FireAlarm.AppendLine(temp_level_name + "，无消火栓箱");
                            stringBuilder_not_pass_FireAlarm.Append(temp_level_name + "  ");
                            H00018_pass = false;
                        }
                    ...
                    //输出消火栓箱的stringbuilder
                    if (!(stringBuilder_pass_FireAlarm.Length > 0 && !"null".Equals(stringBuilder_pass_FireAlarm.ToString()) && !"".Equals(stringBuilder_pass_FireAlarm.ToString())))
                    {
                        stringBuilder_pass_FireAlarm.AppendLine("无");
                    }
                    if (!(stringBuilder_not_pass_FireAlarm.Length > 0 && !"null".Equals(stringBuilder_not_pass_FireAlarm.ToString()) && !"".Equals(stringBuilder_not_pass_FireAlarm.ToString())))
                    {
                        stringBuilder_not_pass_FireAlarm.AppendLine("无");
                    }

                    if (exist_FireAlarm == false)
                    {
                        stringBuilder.Append("已设置“消火栓箱”的楼层：" + "\n" + stringBuilder_pass_FireAlarm);
                        stringBuilder.Append("未设置“消火栓箱”的楼层：" + "\n" + stringBuilder_not_pass_FireAlarm + "\n");
                    }
                    else
                    {
                        stringBuilder.Append("已设置“消火栓箱”的楼层：" + "\n" + stringBuilder_pass_FireAlarm);
