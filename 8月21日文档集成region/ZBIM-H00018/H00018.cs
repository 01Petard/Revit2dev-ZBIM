...
namespace ZBIM
{
    ...
    public class H00018 : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            #region 步骤1.过滤获得所有消火栓、所有消火管和所有消火栓箱
            ...
            foreach (var ele in elementsGrids)
            {
                    ...
                    !ele.Name.Contains("测"))
                {
                    ...
                    //stringBuilder.AppendLine(Utils.StartSubString(ele.Name, 0, "-"));
            ...
            {
                //stringBuilder.AppendLine(Utils.StartSubString(elementsGrids[i].Name, 0, "-"));
                for (jj = 0; jj < ii; jj++)
                {
                    if (!"null".Equals(elementsGridsSort[jj].Name) && !"".Equals(elementsGridsSort[jj].Name) && !"null".Equals(elementsGridsSort[jj + 1].Name) && !"".Equals(elementsGridsSort[jj + 1].Name))
                    {
                        try
                        {
                            string j1_str = Utils.StartSubString(elementsGridsSort[jj].Name, 0, "-");
                            string j2_str = Utils.StartSubString(elementsGridsSort[jj + 1].Name, 0, "-");
            ...
            foreach (var ele in elementsGrids)
            {
                ...
                {
                    if (!FireGate_buildNum.ContainsKey(build_num_might))
                    {
                        FireGate_buildNum.Add(build_num_might, new List<Element>());
                    }
                    if (!FirePipe_buildNum.ContainsKey(build_num_might))
                    {
                        FirePipe_buildNum.Add(build_num_might, new List<Element>());
                    }
                    if (!FireAlarm_buildNum.ContainsKey(build_num_might))
                    {
                        FireAlarm_buildNum.Add(build_num_might, new List<Element>());
                    }
                }
            }
            ...
            #region 步骤3.获取每栋楼每层的消火栓、消火管、消火栓箱坐标
                    ...
                    !item.Name.Contains("测"))
                {
                    ...
                    else //如果前后两个轴网发生变化，说明已经切换到下一栋楼了，打印输出一下这栋楼的占地方位
                    {
                        ...
                        foreach (var FirePipe in FirePipeList)
                        {
                            ...
                            FirePipeXLength = Math.Round(Utils.FootToMeter(Math.Abs(FirePipeMaxX - FirePipeMinX)), 2);
                            FirePipeYLength = Math.Round(Utils.FootToMeter(Math.Abs(FirePipeMaxY - FirePipeMinY)), 2);
            ...
            #region 步骤4.从模型立面获得每层标高、获得建筑楼栋数量
...
/*                StringBuilder stringBuilder_notpass_alarm = new StringBuilder();
                ...
                {
                    #region 高度在21-27m的楼层，每层判断有无消防立管、消火栓
                    foreach (Level litem in build_upfloorsNumList) //从这里开始遍历建筑的楼层
                    {
                        ...
                        {
                            level_name = Utils.SubBetweenString(level_name, "#", "F", false, true, true);
                        }
                        level_name_num = Convert.ToInt16(Utils.StartSubString(level_name, 0, "F"));
                        //判断消火栓、消防立管
                        ...
                        if (exist_FireGate == true && exist_FirePipe == true)
                        {
                            ...
                            //string_pass_result.Add("F，有消火栓：" + pass_FireGate + "，有消防立管：" + pass_FirePipe + "\n");
                        }
            ...
            //打印输出
            Utils.PrintLog(stringBuilder.ToString(), "H00017", document);
