...
namespace ZBIM
{
    ...
    public class H00101 : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            //====后台切换视图====
            View root_View = document.ActiveView;
            View v = null;
            //初始化一个视图收集器，收集所有的平面楼层视图
            ElementCategoryFilter ViewFilter = new ElementCategoryFilter(BuiltInCategory.OST_Views);
            FilteredElementCollector ViewCollector = new FilteredElementCollector(document).WherePasses(ViewFilter).OfClass(typeof(ViewPlan));
            foreach (var item in ViewCollector)
            {
                if (item != null)
                {
                    v = (View)item;
            ...
            //数据收集完毕，后台视图跳转返回
            document = root_View.Document;
            #endregion

            #region 步骤1.获得墙体的BoundingBox、栏杆的BoundingBox
            ...
            */
            ...
            {
                #region 获得栏杆的BoundingBox、为栏杆的走向、起始点坐标赋值
                ...
                try
                {
                    ...
                    foreach (Curve curve in railing.GetPath())
                    {
                        if (curve is Line line) //判断Curve对象是否为Line对象
                        {
                            #region 步骤2.遍历每个栏杆的投影线段，获得线段的走向、长度
                            line = (Line)curve;
                            X_direction = Get_Right_Number(line.Direction.X);
                            Y_direction = Get_Right_Number(line.Direction.Y);
                            ...
                            */
                            ...
                            if (line_length > 0.2)
                            {
                                ...
                                {
                                    #region 获得墙的BoundingBox、为栏杆的走向、起始点坐标赋值
                                    ...
                                    */
                                    ...
                                    #region 步骤3.扩展栏杆投影的每条线段，判断扩展前与扩展后的线段是否与墙碰撞
                                    //扩展前计算是否碰撞
                                    ...
                                    #region 步骤4.栏杆的投影线段在扩展前和扩展后都没碰撞，则认为栏杆符合要求，反之都不符合要求
