...
namespace ZBIM
{
    ...
    public class Model47 
    {
        ...
        //获得每栋建筑中某种类型的面积分区
        public static Dictionary<string, List<Area>> GetBuildArea(Autodesk.Revit.DB.Document document, UIDocument uIDocument, string AreaTypeName)
        {
            //======获取防火分区=====
            //初始化
            Dictionary<string, List<Area>> FireArea_buildNum = new Dictionary<string, List<Area>>();
            //获取到所有楼的楼号和BoundingBoxUV
            Dictionary<string, BoundingBoxUV> Build_Dict = Utils.GetBuildingBoundingBoxUV(document, uIDocument);
            //获取到所有楼的楼号
            List<string> BuildNums = Build_Dict.Keys.ToList();
            foreach (string BuildNum in BuildNums)
            {
                FireArea_buildNum.Add(BuildNum, new List<Area>());
            }
            /*
            Dictionary<string, List<Area>> FireArea_buildNum = new Dictionary<string, List<Area>>
            {
        ...
        public static Dictionary<string, List<Pipe>> GetBuildPipe(Autodesk.Revit.DB.Document document, UIDocument uIDocument, List<Pipe> PipeList, List<Element> elementsGridsSort)
        {
            ...
            ViewPlan workView = get2DView(document, "#1F");
            ...
            foreach (string BuildNum in BuildNums)
            {
                Pipe_buildNum.Add(BuildNum, new List<Pipe>());
            }
            /*
            Dictionary<string, List<Pipe>> Pipe_buildNum = new Dictionary<string, List<Pipe>>
            {
            ...
            */
            ...
            foreach (var item in elementsGridsSort)
            {
                    ...
                    !item.Name.Contains("测"))
                {
                    ...
                    {
                        BoundingBoxXYZ buildBoundingBoxXYZ = item.get_BoundingBox(workView);//计算轴网建筑的占地范围
                    ...
                    else //如果前后两个轴网发生变化，说明已经切换到下一栋楼了，打印输出一下这栋楼的占地方位
                    {
                        ...
                        {
                            BoundingBoxXYZ FirePipeBoundingBoxXYZ = Pipe.get_BoundingBox(workView);
                            XYZ FirePipeXYZMax = FirePipeBoundingBoxXYZ.Max; //取到Max元组
                            XYZ FirePipeXYZMin = FirePipeBoundingBoxXYZ.Min; //取到Min元组
                    ...
                    {
                        BoundingBoxXYZ FirePipeBoundingBoxXYZ = FirePipe.get_BoundingBox(workView);
                        XYZ FirePipeXYZMax = FirePipeBoundingBoxXYZ.Max; //取到Max元组
                        XYZ FirePipeXYZMin = FirePipeBoundingBoxXYZ.Min; //取到Min元组
                        PipeMaxX = FirePipeXYZMax.X;
                        PipeMinX = FirePipeXYZMin.X;
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            //==========防火分区FireArea==========
            Dictionary<string, List<Area>> FireArea_buildNum = GetBuildArea(document, uiDocument, "防火分区");
            //查看每栋楼防火分区
            //ShowBuildArea(FireArea_buildNum, stringBuilder);
            //Utils.GetFireAreas(document);
            ...
            //==========找到每个轴网范围内的喷淋管道==========
            Dictionary<string, List<Pipe>> FirePipe_buildNum = GetBuildPipe(document, uiDocument, FirePipeList, elementsGridsSort);
            //查看每栋楼的喷淋管
